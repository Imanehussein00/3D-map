...
export default function MasterPlanSVG({ onPropertySelect, selectedPropertyId }) {
  ...
  useEffect(() => {
    // Robust parsing using text/html to handle fragmented SVG strings
    const parser = new DOMParser();
    const doc = parser.parseFromString(fullSvgContent, 'text/html');

    // Parse Polygons
    const polys = Array.from(doc.querySelectorAll('polygon')).map(p => {
      const name = p.getAttribute('data-name');
      const pointStr = p.getAttribute('points') || '';

      // Skip if it doesn't have a name or invalid points
      if (!name || !pointStr.trim()) return null;

      const tokens = pointStr.replace(/,/g, ' ').trim().split(/\s+/);
      if (tokens.length < 6) return null; // Need at least 3 points (6 tokens)

      const coords = [];
      for (let i = 0; i < tokens.length; i += 2) {
        if (tokens[i] && tokens[i + 1]) {
          coords.push([-parseFloat(tokens[i + 1]), parseFloat(tokens[i])]);
        }
      }
      return {
        id: name,
        ...
        area: p.getAttribute('data-area'),
        availability: p.getAttribute('data-availability'),
        positions: coords,
      };
    }).filter(p => p && p.positions && p.positions.length > 0);

    // Parse 360 Circles + Paths for icons
    const panos = Array.from(doc.querySelectorAll('g[data-name^="360-"]')).map(g => {
      ...
      // We want to normalize the path relative to the circle center
      // But for simplicity, we'll just keep the absolute cx/cy for state
      // and use a clever viewBox in the custom icon.
      return {
        id: g.getAttribute('data-name'),
        center: [-cy, cx],
        radius: r,
  ...
  // Custom 360 Icon Generator
  const create360Icon = (pano) => {
    const size = 48; // increased visual size
    const viewBoxSize = pano.radius * 2.1;
    const content = `
      <svg viewBox="${pano.cx - pano.radius - 5} ${pano.cy - pano.radius - 5} ${viewBoxSize} ${viewBoxSize}" xmlns="http://www.w3.org/2000/svg">
        <circle cx="${pano.cx}" cy="${pano.cy}" r="${pano.radius}" fill="#ffffff" fill-opacity="0.9" />
        ${pano.pathData ? `<path d="${pano.pathData}" fill="none" stroke="#000" stroke-linecap="round" stroke-linejoin="round" stroke-width="8" />` : ''}
      </svg>
    `;
    return L.divIcon({
      html: content,
      className: 'custom-360-marker',
      iconSize: [size, size],
  ...
  return (
    <>
      {properties.map(p => {
        ...
        const isReserved = p.availability === 'reserved' || p.availability === 'booked' || p.availability === 'sold';

        let fillColor = '#546e7a'; // Default "Still in progress"
        if (isAvailable) fillColor = '#6b8e23';
        if (isReserved) fillColor = '#cb6d4f';
        ...
        return (
          <Polygon
            ...
            pathOptions={{
              ...
              opacity: 0,
              fillColor: fillColor,
              fillOpacity: selectedPropertyId === p.id ? 0.111 : 0.45,
            }}
            eventHandlers={{
              click: (e) => {
                L.DomEvent.stopPropagation(e);
                // Auto-zoom on selection
                map.flyToBounds(e.target.getBounds(), { padding: [50, 50], maxZoom: 1 });
      ...
      {markers360.map(m => (
        ...
        >
          <Tooltip direction="top" offset={[0, -10]} permanent={false}>360° Panorama</Tooltip>
        </Marker>
      ))}
    </>
  );
